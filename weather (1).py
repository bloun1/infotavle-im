...
from felles import (
    ...
    YELLOW_SURFACE, RoundedPanel, px, font, emoji_photo,
)
...
class WeatherPanel:
    ...
    Emoji icons are rendered as Twemoji images (via felles.emoji_photo) so they
    never tofu on the Pi; numbers/text stay as normal Tk text."""
    def __init__(self, parent, surface):
        self.surface = surface
        self.rf = RoundedPanel(parent, surface)

        # Icon pixel sizes (device px), matched to the old font sizes.
        self._isz = px(30)   # big current-condition icon
        self._dsz = px(14)   # wind/humidity inline icons
        self._fsz = px(16)   # forecast row icons
        ...
        # Big condition icon (image set in update_display)
        self.icon_label = tk.Label(self.current_frame, bg=surface.bg)
        ...
        # Wind row: fixed 💨 icon + text
        wind_row = tk.Frame(self.details_frame, bg=surface.bg)
        wind_row.pack(anchor='w')
        self._wind_icon = tk.Label(wind_row, bg=surface.bg)
        self._set_icon(self._wind_icon, '💨', self._dsz)
        self._wind_icon.pack(side='left', padx=(0, px(3)))
        self.wind_label = tk.Label(wind_row, text="", font=font(11, 'normal'),
                                   bg=surface.bg, fg=WHITE)
        self.wind_label.pack(side='left')

        # Humidity row: fixed 💧 icon + text
        hum_row = tk.Frame(self.details_frame, bg=surface.bg)
        hum_row.pack(anchor='w')
        self._hum_icon = tk.Label(hum_row, bg=surface.bg)
        self._set_icon(self._hum_icon, '💧', self._dsz)
        self._hum_icon.pack(side='left', padx=(0, px(3)))
        self.humidity_label = tk.Label(hum_row, text="", font=font(11, 'normal'),
                                       bg=surface.bg, fg=WHITE)
        ...
        for _ in range(4):
            ...
            icon_lbl = tk.Label(row, bg=surface.bg)   # image set in update_display
            icon_lbl.pack(side='left', padx=px(2))
    ...
    def _set_icon(self, label, emoji, size):
        """Render emoji as an image onto label. emoji_photo caches the photo and
        label.image holds a strong ref, so nothing is GC'd and the set of photos
        stays bounded even though update_display runs every cycle."""
        photo = emoji_photo(emoji, size, bg=self.surface.bg)
    ...
    def update_display(self):
        ...
        self._set_icon(self.icon_label, weather['icon'], self._isz)
        self.temp_label.config(text=weather['temp'])
        # Icons are now images to the left; the text labels hold just the values.
        self.wind_label.config(text=weather['wind'])
        self.humidity_label.config(text=weather['humidity'])
        ...
        for i, (time_lbl, icon_lbl, temp_lbl) in enumerate(self.forecast_labels):
            if i < len(weather['forecast']):
                ...
                self._set_icon(icon_lbl, fc['icon'], self._fsz)
